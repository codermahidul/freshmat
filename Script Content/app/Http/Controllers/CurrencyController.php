...
class CurrencyController extends Controller
{
    ...
    public function insert(Request $request){
        ...
        if ($currencyExists) {
        return redirect()->back()->withErrors(['currency' => trans('The selected currency exist in our records.')]);
        }
        ...
        toast(trans('Currency added successfull!'), 'success')->width('350');
    ...
    public function update(Request $request, $id){
        ...
        toast(trans('Currency update successfull!'), 'success')->width('350');
        return redirect()->route('currency');
    }
    ...
    function delete($id)
    {
        try {
            ...
            if ($default == 'yes') {
                return response()->json(['status' => 'have', 'message' => trans('Deletion of the default currency is not allowed.')]);
            } else {
                Currency::where('id',$id)->delete();
                return response()->json(['status' => 'success', 'message' => trans('Currency Deleted Successfully.')]);
            }
        } catch (\Throwable $th) {
