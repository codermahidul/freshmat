    ...
    <div class="col-md-12">
        <div class="card">
            <div class="card-header">
                <h2 class="card-title">{{ __('Processing Orders') }}</h2>
            </div>
            <div class="card-body">
        ...
        <form action="{{ route('orderStatus', $order->id) }}" method="post">
          ...
          <div class="form-group">
            ...
            <select name="payment" id="payment" class="form-control">
              <option {{ ( $order->payment == 'pending' ) ? 'selected' : '' }} value="pending">{{ __('Pending') }}</option>
              <option {{ ( $order->payment == 'success' ) ? 'selected' : '' }} value="success">{{ __('Success') }}</option>
            </select>
          </div>
          <div class="form-group">
            ...
            <select name="status" id="status" class="form-control">
              ...
              <option {{ ( $order->status == 'delevery-in-process' ) ? 'selected' : '' }} value="delevery-in-process">{{ __('Delevery In Process') }}</option>
              <option {{ ( $order->status == 'complete' ) ? 'selected' : '' }} value="complete">{{ __('Complate') }}</option>
              <option {{ ( $order->status == 'cancel' ) ? 'selected' : '' }} value="cancel">{{ __('Canceled') }}</option>
            </select>
          </div>
      </div>
